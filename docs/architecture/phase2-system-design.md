...
│  - Server-rendered           │  - Render biểu đồ, heatmap async │
└──────────────┬───────────────┴──────────────┬───────────────────┘
...
│  ┌─────────────────────────────────────────────────────────┐    │
│  │                    ROUTES LAYER                           │    │
│  │  Page: /dashboard/{repo_id}*, /repositories*, /explore   │    │
│  │  API:  /api/v1/analytics/{repo_id}*, /api/v1/insights*,   │    │
│  │        /api/v1/explore/trending                          │    │
│  │  Auth: /auth/github/login, /auth/github/callback, etc.   │    │
│  └──────────────────────────┬──────────────────────────────┘    │
│                              │                                    │
│  ┌──────────────────────────▼──────────────────────────────┐    │
│  │                   SERVICE LAYER                           │    │
│  │  AuthService │ SyncService │ AnalyticsService │           │    │
│  │  InsightsService │ ExploreService                         │    │
│  │  - Business logic, Orchestration, In-Memory TTL Cache     │    │
│  └───────┬──────────────────┬──────────────────────────────┘    │
│           │                  │                                    │
│  ┌────────▼────────┐  ┌─────▼─────────────────────────────┐    │
│  │  CLIENTS LAYER  │  │        REPOSITORY LAYER            │    │
│  │  GitHubClient   │  │  UserRepo │ CommitRepo │ PRRepo    │    │
│  │  explore_client │  │  IssueRepo │ RepositoryRepo        │    │
│  │  - HTTP calls   │  │  - CRUD + Upsert                   │    │
│  │  - Rate limits  │  │  - SQL aggregation metrics         │    │
│  └────────┬────────┘  └─────────────────┬──────────────────┘    │
│           │                              │                        │
└───────────┼──────────────────────────────┼────────────────────────┘
            │                              │
...
│   Hacker News API  │          │  PostgreSQL (prod)     │
└───────────────────┘          └───────────────────────┘
```
...
│                                 APP MODULES                                 │
├──────────────┬──────────────┬──────────────────┬──────────────┬─────────────┤
│     AUTH     │     SYNC     │    ANALYTICS     │   EXPLORE    │    CORE     │
│ (routes/jwt) │ (jobs/queue) │ (metrics/charts) │  (AI tools)  │ (db/config) │
└──────────────┴──────────────┴──────────────────┴──────────────┴─────────────┘
```
...
git-analytics/
├── app/
│   ├── main.py                         # FastAPI app factory, middleware, exception registration
│   ├── core/
│   │   ├── __init__.py
│   │   ├── config.py                   # Settings from .env (Pydantic BaseSettings)
│   │   ├── security.py                 # AES token encryption/decryption, cookie parser
│   │   ├── exceptions.py              # Domain exception hierarchy (AppException)
│   │   ├── exception_handlers.py      # central exception mapping to JSON responses
│   │   └── logging.py                 # Structured logging, trace_id thread-local context
│   ├── db/
│   │   ├── __init__.py
...
│   ├── models/
│   │   ├── __init__.py
│   │   ├── user.py                     # User table (encrypted_github_token, github_id)
│   │   ├── repository.py              # Repository table (default_branch, last_synced_at)
│   │   ├── contributor.py             # Contributor table (github_login, email fallback)
│   │   ├── commit.py                   # Commit table
│   │   ├── pull_request.py            # PullRequest table (is_merged boolean)
│   │   └── issue.py                    # Issue table (labels JSON array)
│   ├── repositories/
│   │   ├── __init__.py
│   │   ├── user_repo.py
│   │   ├── repository_repo.py
...
│   ├── services/
│   │   ├── __init__.py
│   │   ├── auth_service.py            # OAuth flows & user mapping
│   │   ├── sync_service.py            # Rate limits & data sync orchestration
│   │   ├── analytics_service.py       # Standard page analytics SQL Aggregations
│   │   ├── insights_service.py        # Advanced developer behavioral intelligence
│   │   └── explore_service.py         # Parallel ecosystem aggregator with TTL cache
│   ├── clients/
│   │   ├── __init__.py
│   │   ├── github_client.py           # GitHub REST API client
│   │   └── explore_client.py          # Ext API client for trending & HN feeds
│   ├── routes/
│   │   ├── __init__.py
│   │   ├── auth.py                    # OAuth login callback & sign out
│   │   ├── repositories.py            # Import raw repos, switch repos, sync triggers
│   │   ├── dashboard.py               # Pages router (Overview, Commits, PR, Issue, Insights)
│   │   ├── api_analytics.py           # API endpoints (/api/v1/analytics/{repo_id}/*)
│   │   ├── api_insights.py            # API endpoints (/api/v1/insights/{repo_id})
│   │   ├── explore.py                 # Explore page (/explore) & API (/api/v1/explore/trending)
│   │   └── health.py                  # Server health checks (/health)
│   └── schemas/
│       ├── __init__.py
│       ├── response.py                # success_response & error_response structures
│       ├── repository.py
│       └── analytics.py
├── templates/
│   ├── base.html                       # Global Linear/Vercel dark shell (sidebar + topbar)
│   ├── login.html                      # OAuth login entrypoint
│   ├── repositories.html               # Repository selection and import manager
│   ├── dashboard_overview.html        # Overview telemetry charts & cards
│   ├── dashboard_insights.html        # Streaks, contribution heatmap, and keywords
│   ├── dashboard_commits.html         # Commit frequency & productivity
│   ├── dashboard_pull_requests.html   # Merged/open charts & average merge ages
│   ├── dashboard_issues.html          # Open/closed & labels distributions
│   └── explore.html                    # Hacker News & AI tools feed
├── static/
│   └── .gitkeep                        # Static directory placeholder (CSS is embedded in base)
├── migrations/                          # Alembic migrations database histories
│   ├── versions/
│   └── env.py
├── tests/
│   ├── test_auth.py                    # Auth flow mocks & token crypt check
│   ├── test_repositories.py            # Database CRUD test cases
│   ├── test_routes_repositories.py     # Endpoint router unit tests
│   ├── test_sync_service.py            # Sync increments, paginator, rate limit logic
│   ├── test_analytics_api.py           # Analytics API payloads
│   ├── test_insights_api.py            # Insights API payloads
│   ├── test_explore_api.py             # Explore caches & aggregate logic
│   ├── test_github_client.py           # Mock httpx responses to GitHub endpoints
│   ├── test_models.py                  # ORM constraints & unique checks
│   └── test_health.py                  # Standard app health route verification
├── .env.example                         # Env credentials template
├── .gitignore
├── alembic.ini
├── requirements.txt
└── README.md                # Token encryption/decryption, cookie signing
