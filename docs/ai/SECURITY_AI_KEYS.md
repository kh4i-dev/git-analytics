...
4. **Không bao giờ trả lại khóa thô cho Client**: API lấy cấu hình cài đặt (`/api/v1/settings`) chỉ trả về trạng thái `"has_key": true` and nhãn mặt nạ `"masked_key": "********"`. Tuyệt đối không bao giờ trả về chuỗi API Key thô ban đầu.
